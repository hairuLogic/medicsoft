...
session_start();
	...
	if($chgCode!='' || $searchField2!=""){
		...
		$SQL = "SELECT COUNT(DISTINCT p.chgcode) AS COUNT FROM chgprice AS p, chgmast AS a WHERE p.chgcode=a.chgcode AND p.effdate < CURDATE() AND p.compcode ='$compcode'";
	}
	...
	if($chgCode!='' || $searchField2!=""){
		...
		$SQL = "SELECT p.chgcode,p.effdate,p.amt1,p.amt2,a.description,a.brandname,a.chgtype,a.chggroup FROM chgprice AS p, chgmast AS a WHERE p.chgcode=a.chgcode AND p.compcode ='$compcode' AND p.effdate = ( SELECT MAX(effdate) FROM chgprice WHERE chgcode = p.chgcode AND p.compcode ='$compcode' AND effdate < CURDATE()) LIMIT $start , $limit";
	}
	$result=mysql_query( $SQL )or die("Couldn't execute query.".mysql_error());
