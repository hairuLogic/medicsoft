...
session_start();
	...
	$sql = "SELECT COUNT(memberno) AS count FROM membership WHERE memberno in (SELECT memberno FROM membership WHERE compcode='{$compcode}'".$addSql.")";
	...
	$SQL = "SELECT MemberNo,Name,Newic,telhp,DOB,Sex,Category,Address1,Address2,Address3,OffAdd1,OffAdd2,OffAdd3,telh,telo,mrn,pkgcode,agentcode
 FROM membership WHERE memberno in (SELECT memberno FROM membership WHERE compcode='{$compcode}'".$addSql.") order by MemberNo,mrn LIMIT $start , $limit";
