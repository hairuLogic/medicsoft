...
<html>
        ...
        <script>
            ...
            $(function(){
                ...
                $("#but_order").click(function(){
					...
					$("#tablePkgpat").jqGrid().setGridParam({url:'tableList/pkgpat.php?MemberNo='+MemberNo+"&pkgcode="+pkgcode,datatype:'xml'}).trigger("reloadGrid");                      
                });
                ...
                $("#tablePkgpat").jqGrid({		
                    ...
                    colModel:[
						...
						{label:'Remarks',name: 'remark',index: 'remark', width:300},
                        {label:'Package Quantity',name: 'pkgqty',index: 'pkgqty', width: 150},
                        {label:'Balance',name: 'balance',index: 'balance', width: 150},
                    ],
                    viewrecords: true,
                    height: "100",
                    autowidth: true,
                ...
                function test(cellvalue, options, rowObject){
                    ...
                    if (cellvalue==="1"){
                        ...
                        return '<div class="menuPulldown" onClick="deleteEpisode('+options.rowId+','+mrn+');" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');" style="padding-left: 14px;" title="Delete Episode"><span class="ui-icon ui-icon-trash"></span></div>';
                    }
                    else {
                        return "";
                    }
				...
				$("#chgmastList").jqGrid({		
                    ...
                    colModel:[
						...
						{label:'Remarks',name: 'remark',index: 'remark', width:1,hidden:true},
                        {label:'Package Quantity',name: 'pkgqty',index: 'pkgqty', width: 150,hidden:true},
                        {label:'Balance',name: 'balance',index: 'balance', width: 150,hidden:true},
                    ],
                    rowNum:10,
                    viewrecords: true,
                    height: "150",
                        ...
                        if($.trim($("#chgCode").val()).length>2){
                            $.post('process/checkChgCode.php',{chgCode:$("#chgCode").val()},function(data){
                                ...
                                if($msg=='empty'){
                                    ...
                                    $("#chgmastList").jqGrid().setGridParam({url:'tableList/pkgpat.php?MemberNo='+MemberNo+"&pkgcode="+pkgcode+"&chgCode="+$("#chgCode").val(),datatype:'xml'}).trigger("reloadGrid");
									/*.jqGrid().setGridParam({url : 'tableList/orderChgMast.php?chgCode='+$("#chgCode").val()+"&pkgcode="+pkgcode,datatype:'xml'}).trigger("reloadGrid");	*/
                                }
                                else{
                ...
                $("#help").click(function(){
                    $("#chgmastList").jqGrid().setGridParam({url:'tableList/pkgpat.php?MemberNo='+MemberNo+"&pkgcode="+pkgcode,datatype:'xml'}).trigger("reloadGrid");
                    $("#dialogList").dialog( "open" );	
                });
                ...
                $("#searchString").on('keyup',  function(e) { //search pt
                    var keyCode = e.keyCode || e.which; 
                    //check for tab n enter key 
                    if (keyCode == 9 || keyCode == 13) { 
                        e.preventDefault(); 
            ...
            function deleteEpisode(rown,mrn){
                $.post('process/episode_p.php',{id:rown,opStatus:'delete',mrn:$("#mrn1").val()},function(data){
                    $msg=$(data).find('msg').text();
                    if($msg=='success'){
						var trf = $("#tableEpisode tbody:first tr:first")[0];
						$("#tableEpisode tbody:first").empty().append(trf);
                        ...
                        $('#dialogText').text("Delete item in transaction first");
                        $( "#dialogAlert" ).dialog( "open" );	
                    }
                    else{
                        alert();
    ...
    <body>
        <span id="pagetitle">Charge Entry</span>
                    ...
                    <div class="body-inner" style="height: 1000px;">
                        <div id="tdTree" >
                            <div class="alongdiv" style="margin-top: 0;">
                                ...
                                <div class="bodydiv" style="height: 320px;padding: 0;" ><!--overflow-y:scroll-->
                                            ...
                                            <table id="tableOrderForm" border="0" cellspacing="0" cellpadding="0" >                                       			<tr>
                                                ...
                                                <tr><form id="chgTrxForm" method="post">
                                                        ...
                                                        <td style="display:none"> 
                                                            ...
                                                            <input name="chgDate" class="iForm" id="chgDate" type="text" value="chgDate" readonly />
