<?php
	...
	$table=$_GET['table'];
	if(!isset($_GET['table'])){
		echo 'Need to open this page from main page';
	}
?>
<!DOCTYPE html>
...
$(function(){
	...
	$('#toggletable input[type=radio]').click(function(){
		...
		$("#grid").jqGrid('setGridParam',{url:servername+"hisdb/allsetupres.php?table="+tabledecider()}).trigger("reloadGrid");
		
	});
	$("#grid").jqGrid({
			url:servername+"hisdb/allsetupres.php?table="+tabledecider(),
			datatype: "xml",
			height: 250,
			width:500,
	...
	function dooperation(oper){
		...
		$.get('allsetupaddnupd.php',{oper:$('#updoradd').val(),code:$('#code').val(),des:$('#desc').val(),table:tabledecider(),id:$('#rowid').val(),username:$('#username').val()},function(data){
...
#in{
	...
	box-shadow:0px 20px 3px rgba(50, 50, 50, 0.75);
}
#toggler{
	font-size:16px;
	position:absolute;
...
<body>
	...
	<div id="formmenu">
    	<div id="menu">
            ...
            <button type="button" id="delbut" ></button>
        ...
        <input type="hidden" id="urldet" value='<?php echo $table;?>'/>
        <input type="hidden" id="username" value="<?php echo $_SESSION['username'];?>"/>
        <input type="hidden" id="updoradd"/>
        <input type="hidden" id="rowid"/>
        <div class="alongdiv">
        <div class="smalltitle"><p>Table form</p><div id="toggler">Change Table</div></div>
        <div class="bodydiv">
        <div id="toggletable">
        <div id="in">
          <p>
        ...
        <tr><th><label>Code</label></th><th><label>Description</label></th><th><label>Created By</label></th><th><label>Created Date</label></th></tr>
        <tr><td width="10%"><input type="text" id="code" name="Code" class='req'/></td>
