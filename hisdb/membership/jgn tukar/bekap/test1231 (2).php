...
if(isset($_GET['searchField'])&&$_GET['searchString']!=''){
	...
	if($searchOper=='eq'){
		$result=mysql_query("SELECT COUNT(DISTINCT memberno) AS count FROM membership WHERE $searchField = '$searchString' and compcode='$compid'");
		$result2=mysql_query("SELECT DISTINCT memberno FROM membership WHERE $searchField = '$searchString' and compcode='$compid'");
	}if($searchOper=='ne'){
		$result=mysql_query("SELECT COUNT(DISTINCT memberno) AS count FROM membership WHERE $searchField != '$searchString' and compcode='$compid'");
		$result2=mysql_query("SELECT DISTINCT memberno FROM membership WHERE $searchField != '$searchString' and compcode='$compid'");
	}if($searchOper=='lk'){
		$result=mysql_query("SELECT COUNT(DISTINCT memberno) AS count FROM membership WHERE $searchField LIKE '%$searchString%' and compcode='$compid'");
		$result2=mysql_query("SELECT DISTINCT memberno FROM membership WHERE $searchField LIKE '$searchString' and compcode='$compid'");
	}
}else{
	$result = mysql_query("SELECT COUNT(DISTINCT memberno) AS count FROM membership where compcode='$compid'");
	$result2 = mysql_query("SELECT DISTINCT memberno FROM membership where compcode='$compid'");	
}
