...
$(function(){
	...
	$("#grid").jqGrid({
			...
			height: 250,
			width:500,
			colNames: ['Agent Code','Name','Created by','Date Created'],
			colModel: [
				{name: 'Code',index: 'agentCode', width: 60},
				{name: 'name',index: 'name'},
				{name: 'Lastupdate',index: 'adddate'},
				{name: 'Lastuser',index: 'addby'},
			],
			altRows: true,
			altclass: 'zebrabiru',
			multiselect: true,
	...
	function enabletextfield(){
		$('input[type=text][id=code],[id=desc],select').attr('disabled',false);
		$('#code').focus();
	}
	function disabletextfield(){
		$('#formtable input[type=text],select').attr('disabled',true);
	}
	function basemenu(){
		disabletextfield()
...
<body><?php include("../../include/header.php")?><span id="pagetitle">Bill Setup</span>
	...
	<div id="formmenu">
       ...
       <div class="bodydiv">
        ...
        <tr><th style="padding:0 5px;"><label>Agent Code</label></th><th><label>Name</label></th><th><label>Created By</label></th><th><label>Created Date</label></th></tr>
        <tr><td width="10%" style="padding:0 5px;"><input type="text" id="code" name="Code" class='req'/></td>
