...
if(isset($_GET['searchField'])&&$_GET['searchString']!=''){
		if($searchOper=='eq'){
			$SQL="select MRN, Name,Newic,telhp,DOB,Sex,idnumber,address1,address2,address3,OffAdd1,OffAdd2,OffAdd3,telh,telo from patmast WHERE $searchField = '$searchString' and compcode='$compid' ORDER BY $sidx $sord LIMIT $start , $limit ";
		}if($searchOper=='ne'){
			$SQL="select MRN, Name,Newic,telhp,DOB,Sex,idnumber,address1,address2,address3,OffAdd1,OffAdd2,OffAdd3,telh,telo from patmast WHERE $searchField != '$searchString' and compcode='$compid' ORDER BY $sidx $sord LIMIT $start , $limit";
		}if($searchOper=='lk'){
			$SQL="select MRN, Name,Newic,telhp,DOB,Sex,idnumber,address1,address2,address3,OffAdd1,OffAdd2,OffAdd3,telh,telo from patmast WHERE $searchField LIKE '%$searchString%' and compcode='$compid' ORDER BY $sidx $sord LIMIT $start , $limit ";
		}
}else{
	$SQL = "select sysno,MRN, Name,Newic,telhp,DOB,Sex,idnumber,address1,address2,address3,OffAdd1,OffAdd2,OffAdd3,telh,telo from patmast where compcode='$compid' ORDER BY $sidx $sord LIMIT $start , $limit";
}
