...
function newPopup(y,x) {
    if (!myWindow[x])
    {
        myWindow[x]=window.open(y,x);
    }
    else
    {
        if (myWindow[x].closed)
        { 
            myWindow[x]=window.open(y,x);
        }
        else
...
function newPopup(y,x) {
    ...
    {
        var str1='myWindow';
        windowId=str1.concat(x);
        windowId=window.open(y,x);
    }
    else
    {
        if (windowId.closed)
...
<?php
    ...
    function menu($rowX,$x,$class,$compcode){
        ...
        else
        {
            ...
            echo "<li onclick='JavaScript:newPopup(\"".$rowX['url']."\",\"".$rowX['programid']."\");'><a  href=''>".$rowX["programname"]."</a></li>"; 
        }
        $x = $x-1;
        return $x;
    }
