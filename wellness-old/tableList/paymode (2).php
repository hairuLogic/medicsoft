...
$addSql="";
	...
	if($paymode!='' || $searchField2!=""){
		if($searchField2=="chgcode"){
			$SQL="SELECT paymode,description FROM paymode WHERE compcode ='$compcode'  AND paymode like '%$searchString2%' order by paymode LIMIT $start , $limit";
		}	
		else{
			$SQL="SELECT paymode,description FROM paymode WHERE compcode='$compcode' {$addSql} order by paymode LIMIT $start , $limit";
		}		

	}
	else{
		$SQL = "SELECT paymode,description FROM paymode WHERE compcode ='$compcode' order by paymode LIMIT $start , $limit";
	}
