...
<html>
    <head>
        ...
        <script>
		$(function(){
			var agent,memberno,lastsel,amtout;
			var mydata = [];
			var now = new Date();
			...
			$("#tableActivity").jqGrid({				
				...
				colModel:[
					...
					{label:'Amount Pay',name:'amtpay',index:'amtpay',width:90, 
						editable:true, 
						editoptions: { 
							dataEvents: [ { 
								type: 'change', 
				...
				onSelectRow: function(id){
					if(id && id !== lastsel) {
						...
						amtout=ret.amtout;  
						$('#tableActivity').jqGrid("editRow",id,true);
						lastsel = id; 
					}
				}
             });
			 function test(e) { 	
				...
				var inputValue = $('#' + lastsel + '_' + e.data['cell']).val(); 
				// form input value 
				mydata[lastsel-1] = {'amtpay':inputValue}; 
				$("#tableActivity").setRowData(lastsel, {amtout: amtout-inputValue}); 
				$("#tableActivity").setCell(lastsel, e.data['cell'],inputValue); 
				//$("#"+lastsel+"_amtpay").val();
			}
